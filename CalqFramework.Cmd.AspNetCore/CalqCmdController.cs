...
public class CalqCmdController : ControllerBase
{
    ...
    public async Task<IActionResult> ExecuteScript()
    {
        try
        {
            ...
            // Special test command to trigger exception for testing
            if (script == "test_exception_trigger")
            {
                throw new InvalidOperationException("Test exception for error caching");
            }
