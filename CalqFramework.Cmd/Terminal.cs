...
public static class Terminal {
    ...
    public static string CMD(string script, TextReader inputReader, TimeSpan? timeout = null) {
        ...
        return new Command(LocalTerminal.Shell, script, new ProcessRunConfiguration(LocalTerminal.ProcessRunConfiguration) { In = inputReader }) { OutputPostprocessor = LocalTerminal.CommandOutputPostprocessor }.Output;
    }
    ...
    public static Task<string> CMDAsync(string script, TextReader inputReader, TimeSpan? timeout = null) {
        var cancellationTokenSource = new CancellationTokenSource(timeout ?? Timeout.InfiniteTimeSpan);
        return new Command(LocalTerminal.Shell, script, new ProcessRunConfiguration(LocalTerminal.ProcessRunConfiguration) { In = inputReader }) { OutputPostprocessor = LocalTerminal.CommandOutputPostprocessor }.GetOutputAsync();
    }
    ...
    public static Command CMDV(string script, TextReader inputReader, TimeSpan? timeout = null) {
        var cancellationTokenSource = new CancellationTokenSource(timeout ?? Timeout.InfiniteTimeSpan);
        return new Command(LocalTerminal.Shell, script, new ProcessRunConfiguration(LocalTerminal.ProcessRunConfiguration) { In = inputReader }) { OutputPostprocessor = LocalTerminal.CommandOutputPostprocessor };
    }
    ...
    public class LocalTerminalConfigurationContext {
        private readonly ICommandOutputPostprocessor _defaultCommandOutputPostprocessor;
        private readonly IShell _defaultShell;
        private readonly ITerminalLogger _defaultTerminalLogger;

        private readonly AsyncLocal<ICommandOutputPostprocessor> _localCommandOutputPostprocessor;
        ...
        public LocalTerminalConfigurationContext() {
            ...
            _defaultCommandOutputPostprocessor = new CommandOutputPostprocessor();
            _defaultTerminalLogger = new TerminalLogger();

            _localShell = new AsyncLocal<IShell>();
            _localCommandOutputPostprocessor = new AsyncLocal<ICommandOutputPostprocessor>();
            _localTerminalLogger = new AsyncLocal<ITerminalLogger>();
        }

        public ICommandOutputPostprocessor CommandOutputPostprocessor {
            get {
                _localCommandOutputPostprocessor.Value ??= _defaultCommandOutputPostprocessor;
                return _localCommandOutputPostprocessor.Value!;
            }
            set => _localCommandOutputPostprocessor.Value = value;
        }
